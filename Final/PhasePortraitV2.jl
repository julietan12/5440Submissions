...
function phaseplot(model, x1lim, x2lim; clines=true, xinit=(), t=(0.0,50.0),
    ...
    if norm
        ...
        #Plot
        plt1 = quiver(x, y, quiver=(u_norm, v_norm), c=:grey, xaxis=(labels[1],
            (x1lim[1], x1lim[2])), yaxis=(labels[2],(x2lim[1], x2lim[2])))
    else
        plt1 = quiver(x, y, quiver=(u.*scale, v.*scale), c=:grey, xaxis=(labels[1],
            (x1lim[1], x1lim[2])), yaxis=(labels[2],(x2lim[1], x2lim[2])))
    end
    ...
    if clines
            ...
            contour!(x1_range, x2_range, v, lw=3, linestyle=:dot, c=:red,
                    levels=[0], colorbar = :none, xaxis=(labels[1], (x1lim[1], x1lim[2])),
                    yaxis=(labels[2],(x2lim[1], x2lim[2])))
    end
    ...
    for i in xinit
        ...
        plot!(sol, vars=(1,2), lw=2, c=:black, legend = :none, xaxis=(labels[1],
            (x1lim[1], x1lim[2])), yaxis=(labels[2],(x2lim[1], x2lim[2])))
    end
